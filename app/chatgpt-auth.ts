...
export type ChatGPTUser={userId:string;displayName:string;email:string;fullName:string|null;role:string};
...
export async function getChatGPTUser():Promise<ChatGPTUser|null>{const token=(await cookies()).get(COOKIE)?.value;if(!token)return null;const rows=await getDb().select({id:usuariosAcesso.id,email:usuariosAcesso.email,funcao:usuariosAcesso.funcao,ativo:usuariosAcesso.ativo}).from(sessoesAcesso).innerJoin(usuariosAcesso,eq(sessoesAcesso.usuarioId,usuariosAcesso.id)).where(and(eq(sessoesAcesso.tokenHash,await hash(token)),gt(sessoesAcesso.expiraEm,new Date().toISOString()))).limit(1);const u=rows[0];if(!u?.ativo)return null;return{userId:u.id,email:u.email,displayName:u.email,fullName:null,role:u.funcao};}
