...
export async function GET(request:Request){
  ...
  const pdf=await createPdf(`PRESTAÇÕES DE CONTAS - ${competencia}`,sections.length?sections:[{heading:"SEM REGISTROS",lines:["Nenhuma prestação encontrada nesta competência."]}]);
  return new Response(pdf.buffer.slice(pdf.byteOffset,pdf.byteOffset+pdf.byteLength),{headers:{"content-type":"application/pdf","content-disposition":`attachment; filename="prestacoes-${competencia}.pdf"`}});
}
