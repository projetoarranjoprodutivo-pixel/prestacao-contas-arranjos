...
export async function GET(request:Request){
  ...
  const [relatorios,usuarios,financeiro]=await Promise.all([
    db.select().from(prestacoes).where(and(eq(prestacoes.associacao,associacao.nome),eq(prestacoes.competencia,competencia))),
    db.select().from(colaboradores),
    db.query.documentosAssociacao.findFirst({where:and(eq(documentosAssociacao.associacao,associacao.nome),eq(documentosAssociacao.competencia,competencia))}),
  ]);
  ...
  const extratos=lista<Arquivo>(financeiro?.extratosJson),notas=lista<Arquivo>(financeiro?.notasFiscaisJson),anexos=relatorios.flatMap(r=>lista<Arquivo>(r.anexosJson));
  ...
  const sections:PdfSection[]=[
    ...
    {heading:"IDENTIFICAÇÃO",lines:[`ASSOCIAÇÃO: ${informado(associacao.razaoSocial||associacao.nome)}`,`CNPJ: ${informado(associacao.cnpj)}`,`MUNICÍPIO: ${informado(associacao.municipio)} / ${informado(associacao.uf)}`,`MÊS DE REFERÊNCIA: ${mesReferencia(competencia)}`,`RESPONSÁVEL PELO PREENCHIMENTO: ${informado(associacao.presidenteNome)}`,`TÉCNICO(S) RESPONSÁVEL(IS): ${tecnicos.join("; ")||"NÃO INFORMADO"}`]},
    {heading:"AGRICULTORES",lines:agricultores.length?agricultores.map((a,i)=>`${i+1}. NOME DO AGRICULTOR: ${a.nome} | MUNICÍPIO: ${a.municipio} | TIPO DE BENEFÍCIO RECEBIDO: ${[...new Set(a.beneficios)].join(", ")||"ASSISTÊNCIA TÉCNICA"} | QUANTIDADE: ${a.quantidade||"NÃO INFORMADO"} | OBSERVAÇÕES: ${a.observacoes.join("; ")||"NÃO INFORMADO"}`):["NENHUM AGRICULTOR REGISTRADO NESTA COMPETÊNCIA."]},
    {heading:"VISITAS TÉCNICAS",lines:visitas.length?visitas.map((x,i)=>`${i+1}. DATA: ${dataBr(x.a.data)} | TÉCNICO RESPONSÁVEL: ${informado(x.u?.nomeCompleto)} | NOME DO AGRICULTOR: ${informado(x.a.agricultor||x.a.beneficiario)} | MUNICÍPIO: ${informado(x.a.municipio)} | ATIVIDADE PRODUTIVA: ${informado(x.a.tipoMuda||x.a.tipoAtividade)} | ENTREGAS: ${x.a.quantidadeMudas?`${x.a.quantidadeMudas} ${informado(x.a.tipoMuda)}`:"NÃO INFORMADO"} | OBSERVAÇÕES: ${informado(x.a.resumo)}`):["NENHUMA VISITA TÉCNICA REGISTRADA NESTA COMPETÊNCIA."]},
    ...
    {heading:"ENTREGAS DE MUDAS",lines:entregas.length?entregas.map((x,i)=>`${i+1}. DATA: ${dataBr(x.a.data)} | MUNICÍPIO: ${informado(x.a.municipio)} | TIPO DE MUDAS: ${informado(x.a.tipoMuda)} | QUANTIDADE: ${informado(x.a.quantidadeMudas)} | RESPONSÁVEL: ${informado(x.u?.nomeCompleto)} | RESULTADO: ${informado(x.a.resumo)}`):["NENHUMA ENTREGA DE MUDAS REGISTRADA NESTA COMPETÊNCIA."]},
    {heading:"AQUISIÇÕES E DOCUMENTOS FISCAIS",lines:notas.length?notas.map((a,i)=>`${i+1}. DATA DA COMPRA: NÃO INFORMADO | INSUMO: NÃO INFORMADO | ITEM ADQUIRIDO: NÃO INFORMADO | QUANTIDADE: NÃO INFORMADO | VALOR UNITÁRIO: NÃO INFORMADO | VALOR TOTAL: NÃO INFORMADO | FORNECEDOR: NÃO INFORMADO | DOCUMENTO FISCAL: ${a.nome} | BENEFICIÁRIOS: NÃO INFORMADO | OBSERVAÇÕES: ARQUIVO INTEGRAL ANEXADO AO FINAL`):["NENHUMA NOTA FISCAL ENVIADA NESTA COMPETÊNCIA."]},
    {heading:"CONTROLE DE HORAS - FOMENTO",lines:relatorios.length?relatorios.map((r,i)=>{const u=nomes.get(r.authUserId);return `${i+1}. NOME DO PROFISSIONAL: ${informado(u?.nomeCompleto)} | CARGO / FUNÇÃO: ${informado(u?.cargo)} | PERÍODO DE REFERÊNCIA: ${mesReferencia(competencia)} | HORAS PREVISTAS: NÃO INFORMADO | HORAS TRABALHADAS: ${(r.totalMinutos/60).toLocaleString("pt-BR",{maximumFractionDigits:2})} | VALOR DA HORA: NÃO INFORMADO | VALOR CORRESPONDENTE: NÃO INFORMADO | OBSERVAÇÕES: ${informado(r.observacoes)}`;}):["NENHUM CONTROLE DE HORAS REGISTRADO NESTA COMPETÊNCIA."]},
  ...
  const base=await createPdf(`PRESTAÇÃO DE CONTAS À ADERES - ${associacao.nome} - ${competencia}`,sections);const pdf=await PDFDocument.load(base);const fonte=await pdf.embedFont(StandardFonts.HelveticaBold);
  ...
  const resultado=await pdf.save();return new Response(resultado.buffer.slice(resultado.byteOffset,resultado.byteOffset+resultado.byteLength),{headers:{"content-type":"application/pdf","content-disposition":`attachment; filename="prestacao-aderes-${associacao.nome.replace(/[^a-zA-Z0-9_-]/g,"-")}-${competencia}.pdf"`}});
}
