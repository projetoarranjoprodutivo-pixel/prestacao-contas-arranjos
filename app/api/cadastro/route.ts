...
export async function POST(request: Request) {
  ...
  try {
    ...
    const cargo=text(form,"cargo");
    let associacoesSelecionadas:string[]=[];try{associacoesSelecionadas=JSON.parse(text(form,"associacoes")||"[]");}catch{}
    associacoesSelecionadas=[...new Set(associacoesSelecionadas.map(nome=>String(nome).trim()).filter(Boolean))];
    if(!cargosMultiplasAssociacoes.has(cargo))associacoesSelecionadas=[text(form,"associacao")].filter(Boolean);
    if(!associacoesSelecionadas.length)return Response.json({message:"Selecione pelo menos uma associação."},{status:400});
    const db=getDb();const associacoesAtivas=await db.select().from(associacoes).where(eq(associacoes.ativo,true));
    if(associacoesSelecionadas.some(nome=>!associacoesAtivas.some(associacao=>associacao.nome===nome)))return Response.json({message:"Selecione somente associações válidas."},{status:400});
    const associacaoValida=associacoesAtivas.find(associacao=>associacao.nome===associacoesSelecionadas[0])!;
    ...
    const values = { authUserId: user.userId, email: user.email, nomeCompleto: text(form,"nomeCompleto"), dataNascimento: text(form,"dataNascimento"), cpf, sexo: text(form,"sexo"), cargo, associacao: associacoesSelecionadas[0], associacoesJson:JSON.stringify(associacoesSelecionadas), municipiosAtendidosJson:JSON.stringify(cargo==="Técnico de campo"?municipiosAtendidos:[]), mei:text(form,"mei")||null, nomeEmpresarial:text(form,"nomeEmpresarial")||null, cftaCrea:text(form,"cftaCrea")||null, cep: text(form,"cep"), endereco: text(form,"endereco"), numero: text(form,"numero"), complemento: text(form,"complemento") || null, bairro: text(form,"bairro"), cidade: text(form,"cidade"), uf: text(form,"uf").toUpperCase(), celular: text(form,"celular"), atendimentosJson: JSON.stringify(atendimentos), superioresJson: cargo==="Técnico de campo"?JSON.stringify(superiores):"{}", documentoKey: primeiro.key, documentoNome: primeiro.nome, documentoTipo: primeiro.tipo, documentosJson: JSON.stringify(documentos), documentoValidade: proximaValidade, atualizadoEm: new Date().toISOString() };
