...
export default function SenhaAdmin({associacoes}:{associacoes:string[]}) {
  ...
  return <div className="mt-6 grid gap-6 xl:grid-cols-2">
    ...
    <section className="rounded-2xl border border-emerald-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-bold">Criar novo usuário e senha</h2><p className="text-sm text-slate-600">Crie acessos para colaboradores, associações ou administradores.</p><form onSubmit={e=>enviar(e,"/api/admin/usuarios/criar",setMensagemUsuario)} className="mt-4 grid gap-4"><label className="text-sm font-bold">E-mail do novo usuário<input name="email" type="email" required autoComplete="off" className={campo}/></label><label className="text-sm font-bold">Tipo de acesso<select name="funcao" required className={campo}><option value="colaborador">Colaborador</option><option value="associacao">Associação — documentos financeiros</option><option value="admin">Administrador — acesso total</option></select></label><label className="text-sm font-bold">Associação<select name="associacao" className={campo}><option value="">Selecione quando o acesso for de associação</option>{associacoes.map(nome=><option key={nome}>{nome}</option>)}</select></label><label className="text-sm font-bold">Senha inicial<input name="senha" type="password" required minLength={8} autoComplete="new-password" className={campo}/></label><label className="text-sm font-bold">Confirmar senha<input name="confirmarSenha" type="password" required minLength={8} autoComplete="new-password" className={campo}/></label><button disabled={enviando} className="rounded-lg bg-emerald-700 px-5 py-3 font-bold text-white disabled:opacity-50">Criar usuário</button>{mensagemUsuario&&<p className="rounded-lg bg-slate-100 p-3 text-sm font-semibold">{mensagemUsuario}</p>}</form></section>
  </div>;
}
